...
run_one_cycle <- function(jx, base_ind) {
  ...
  #for_growth_rpg <- matrix(100 * as.numeric(NG_HENRY[1:ind_cycle, jx]) / as.numeric(CPI_AC[1:ind_cycle, jx]), ncol = 1)  # Real gas price (nominal deflated by US CPI)
  #for_growth_rpg <- 100 * as.numeric(NG_HENRY[1:ind_cycle, jx]) / as.numeric(CPI_AC[1:ind_cycle, jx])  # Real gas price (nominal deflated by US CPI)
  for_growth_rpg <- matrix(100 * as.numeric(NG_HENRY[1:ind_cycle, jx]) / as.numeric(CPI_AC[1:ind_cycle, jx]), ncol = 1)  # Real gas price (nominal deflated by US CPI)
  ...
  #gr_rpg <- 100 * lagn(log(for_growth_rpg), 1)
  #gr_rpg <- 100 * lagn(log(as.matrix(for_growth_rpg)), 1)
  gr_rpg <- 100 * lagn(log(for_growth_rpg), 1)
  ...
  if (isTRUE(run_models$RW_SV)) {
    ...
    latent_h <- as.matrix(rwsv_fit$latent[[1]])
    para <- as.matrix(rwsv_fit$para)
    
    para_col <- function(x, target) {
      col_id <- match(target, colnames(x))
      if (is.na(col_id)) {
        stop(sprintf("RW_SV parameter '%s' not found in stochvol output.", target))
      }
      as.numeric(x[, col_id])
    }
    
    mu_draws <- para_col(para, "mu")
    phi_draws <- para_col(para, "phi")
    sigma_draws <- para_col(para, "sigma")
    last_h_draws <- as.numeric(latent_h[, ncol(latent_h)])
    
    if (!all(is.finite(mu_draws)) || !all(is.finite(phi_draws)) || !all(is.finite(sigma_draws))) {
      stop("RW_SV produced non-finite parameter draws.")
    }
    if (!all(is.finite(last_h_draws))) {
      stop("RW_SV produced non-finite latent volatility draws.")
    }
    ...
    for (i in seq_len(n_draws)) {
      ...
      h_prev <- last_h_draws[i]
      path <- numeric(max(horizons))
      y_prev <- as.numeric(rpg[t])
  ...
  if (isTRUE(run_models$BVAR)) {
    ...
    bvar_fcst_draws <- bvar_fcst$y_pred[seq_len(max(horizons)), which(colnames(bvar_data) == "log_GAS_Price"), , drop = FALSE]
    
    bvar_matrix <- matrix(NA_real_, nrow = n_draws, ncol = max(horizons))
    for (hh in seq_len(max(horizons))) {
  ...
  if (isTRUE(run_models$mixBART)) {
    ...
    mixbart_fcst_draws <- mixbart_fit$fcst[, seq_len(max(horizons)), 7, drop = FALSE]
    mixbart_matrix <- matrix(NA_real_, nrow = n_draws, ncol = max(horizons))
    for (hh in seq_len(max(horizons))) {
  ...
  for (h_eval in horizons) {
    ...
    for (model_name in names(model_draws_by_h)) {
      if (identical(model_name, "mixBART")) {
        # mixBART predicts monthly growth rates (100 * Delta log p_t).
        # Reconstruct each posterior draw of log-level at horizon h by
        # cumulative summation of growth draws and add the last observed log level.
        log_level_draws <- rpg[t] + rowSums(model_draws_by_h[[model_name]][, seq_len(h_eval), drop = FALSE] / 100)
        level_draws <- exp(log_level_draws)
      } else {
        level_draws <- exp(model_draws_by_h[[model_name]][, h_eval])
      }
